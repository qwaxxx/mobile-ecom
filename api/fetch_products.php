...
if ($result->num_rows > 0) {
    ...
    <div class="card h-100 w-100" onclick="showProductModal(' . $row['prod_id'] . ')" style="cursor:pointer;">
        <div class="view overlay">
            <img src="' . htmlspecialchars($row['prod_picture']) . '" class="card-img-top" alt="">
        </div>
        <div class="card-body text-center">
            <h5>' . $row['prod_stock'] . ' in stock</h5>
            <h5><strong>' . htmlspecialchars($row['prod_name']) . '</strong></h5>
            <p>' . htmlspecialchars($row['prod_description']) . '</p>
            <p>₱ ' . number_format($row['prod_price'], 2) . '</p>
        </div>
    </div>
</div>
';
